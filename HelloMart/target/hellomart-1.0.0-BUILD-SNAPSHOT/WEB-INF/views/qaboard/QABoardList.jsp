	...
	<div class="article_wrap">
						...
						<c:if test="${list != null }">
							<c:forEach var="list" items="${list}">
                               ...
                               글을 볼수 없습니다.
									...
									<td class="td_date"><fmt:formatDate value="${list.date}"
		...
		<c:if test="${subjectCount>1 }">
			<div align="center" id="page">
				...
				<c:if test="${startPage > pageBlock}">
					<a
						href="qasearch?searchOption=${searchOption }&keyword=${keyword}&pageNum=${startPage-1}">[이전]</a>
				</c:if>
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<a
						href="qasearch?searchOption=${searchOption }&keyword=${keyword}&pageNum=${i }">[${i }]</a>
				</c:forEach>
				<c:if test="${endPage < subjectCount }">
					<a
						href="qasearch?searchOption=${searchOption }&keyword=${keyword}&pageNum=${startPage+pageBlock}">[다음]</a>
				</c:if>
			</div>
		</c:if>
