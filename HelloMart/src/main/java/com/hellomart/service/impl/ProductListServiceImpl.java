...
public class ProductListServiceImpl implements ProductListService {
	...
	public Map<String, Object> list(Model model) {
		...
		sql = "SELECT no, imagePath, productName, mfCompany, price, score, orderCount, smallCategory From ProductList";
	...
	public String createSQL(Map<String, String[]> paramMap, String select, String table, String mainCategory,
			String smallCategory, Integer offset, Integer limit) {
		StringBuilder sql = new StringBuilder(select);

		Set<String> columns = paramMap.keySet();

		if (!columns.isEmpty()) {
			sql.append(" ").append("NATURAL JOIN").append(" ").append(table);
		}

		sql.append(" ").append("WHERE").append(" ");

		sql.append("mainCategory").append(" = ").append("'").append(mainCategory).append("'");
		...
		for (String column : columns) {
			...
			for (String value : paramMap.get(column)) {
				switch (column) {
				case "search":
					sb.append("ProductName").append(" LIKE ").append("'%").append(value).append("%'");
					break;
				case "price1":
				...
				default:
					...
					if (value.indexOf('~') == -1) {
						sb.append(column).append(" = ").append("'").append(value).append("'");
					} else {
						StringTokenizer tokenizer = new StringTokenizer(value, "~");

						sb.append("(").append(column).append(" >= ").append(filterNumber(tokenizer.nextToken()))
								.append(" and ").append(column).append(" <= ")
								.append(filterNumber(tokenizer.nextToken())).append(")");
					}
				}
			}
			sb.append(")");
		...
		if (offset != null && limit != null && offset != -1) {
			sql.append(" ").append("ORDER BY").append(" ").append("no").append(" ").append("DESC").append(" ")
					.append("LIMIT").append(" ").append(limit).append(" ").append("OFFSET").append(" ").append(offset);
		}
